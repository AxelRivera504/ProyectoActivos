    ...
    public class ClientesController : Controller
    {
        ...
        public ActionResult Index()
        {
            try
            {
                if (Session.Count > 0)
                {
                    var cliente = db.UDP_tbClientes_ListarClientes().ToList();

                    List<tbClientes> CLIENTE = new List<tbClientes>();
                    ...
                    foreach (var item in cliente)
                    {
                        tbClientes tbCliente = new tbClientes
                        {
                            ...
                            clie_CodigoCompania = item.clie_CodigoCompania,
                            clie_NombreCliente = item.clie_NombreCliente,
                            clie_Responsale = item.clie_Responsale,
                            clie_RTNCliente = item.clie_RTNCliente,
                            clie_Lugar = item.clie_Lugar,
                            clie_Telefono = item.clie_Telefono,
                        };

                        CLIENTE.Add(tbCliente);
                    }
        ...
        public ActionResult CheckEditarSession()
        {
            if (Session["Editar"] != null && Session["Editar"].ToString() == "1")
            {
                return Content("1");
            }
            return Content("0");
        ...
        [HttpPost]
        public ActionResult ResetEditarSession()
        {
            Session["Editar"] = "2";
        ...
        [HttpPost]
        public ActionResult ValidarIdentificacionExiste(string RTNCliente)
        {
            try
            {
                if (Session.Count > 0)
                {
                    var identifacionExiste = db.UDP_tbClientes_ValidarRTN(RTNCliente).FirstOrDefault();

                    if (identifacionExiste == 1)
                    {
        ...
        public ActionResult Create([Bind(Include = "clie_CodigoCompania,clie_NombreCliente,clie_RTNCliente,clie_Lugar,clie_Responsale,clie_Telefono")] tbClientes tbCliente)
        {
            try
            {
                if (Session.Count > 0)
                {
                    ...
                    ModelState.Remove("clie_FechaCreacion");
                    ModelState.Remove("usua_Modificacion");
                    ModelState.Remove("clie_Estado");
                    ModelState.Remove("clie_FechaModificacion");
            

                        tbCliente.usua_UsuarioCreacion = int.Parse(Session["usua_Id"].ToString());

                    if (ModelState.IsValid)
                    {
                        var resultado = db.UDP_tbClientes_InsertarClientes(tbCliente.clie_CodigoCompania, tbCliente.clie_NombreCliente, tbCliente.clie_RTNCliente, tbCliente.clie_Lugar, tbCliente.clie_Responsale, tbCliente.clie_Telefono, tbCliente.usua_UsuarioCreacion).FirstOrDefault();

                        if (resultado == 1)
                        {
                            Session["ClienteIngresado12"] = "si";
        ...
        public ActionResult Edit(int? id)
        {
            try
            {
                if (Session.Count > 0)
                {
                    ...
                    var cliente = db.tbClientes.Find(id);

                    tbClientes tbcliente = new tbClientes
                    {
                        clie_Id = cliente.clie_Id,
                        clie_CodigoCompania = cliente.clie_CodigoCompania,
                        clie_NombreCliente = cliente.clie_NombreCliente,
                        clie_Responsale = cliente.clie_Responsale,
                        clie_RTNCliente = cliente.clie_RTNCliente,
                        clie_Lugar = cliente.clie_Lugar,
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "clie_Id,clie_CodigoCompania,clie_NombreCliente,clie_RTNCliente,clie_Lugar,clie_Responsale,clie_Telefono")] tbClientes tbCliente)
        {
            try
            {
                if (Session.Count > 0)
                {
                    ...
                    tbCliente.usua_Modificacion = int.Parse(Session["usua_Id"].ToString());

                    if (ModelState.IsValid)
                    {
                        var resultado = db.UDP_tbClientes_EditarClientes(tbCliente.clie_Id, tbCliente.clie_CodigoCompania, tbCliente.clie_NombreCliente, tbCliente.clie_RTNCliente, tbCliente.clie_Lugar, tbCliente.clie_Responsale, tbCliente.clie_Telefono, tbCliente.usua_Modificacion).FirstOrDefault();

                        if (resultado == 1)
                        {
                            Session["ClienteIngresado"] = "si";
                            return RedirectToAction("Index", "Clientes");
                        }
                        return RedirectToAction("Error500", "Home");
                    }
        ...
        public ActionResult Details(int? id)
        {
            try
            {
                if (Session.Count > 0)
                {
                    ...
                    tbClientes tbclente = db.tbClientes.Find(id);
                    tbUsuarios AS = db.tbUsuarios.Find(tbclente.usua_UsuarioCreacion);
                    tbUsuarios AS1 = db.tbUsuarios.Find(tbclente.usua_Modificacion);
                    Session["UsuarioCrea"] = AS.usua_UsuarioNombre + ' ' + AS.usua_UsuarioApellido;
                    Session["FechaCreUsua"] = tbclente.clie_FechaCreacion;
                    if (tbclente.usua_Modificacion != null && tbclente.usua_Modificacion != 0)
                    {
                        Session["UsuarioMod"] = AS1.usua_UsuarioNombre + ' ' + AS1.usua_UsuarioApellido;
                        Session["FechaModUsua"] = tbclente.clie_FechaModificacion;
                    }
        ...
        [HttpPost]
        public FileResult DescargarExcel(int clie_Id)
        {
            tbClientes tbclente = db.tbClientes.Find(int.Parse(clie_Id.ToString()));
            ...
            using (SqlConnection cn = new SqlConnection("data source=DESKTOP-2FTIJFF\\SQLEXPRESS; initial catalog=ACTIVOS; user id=axel2002; password=axel2002"))
            {
                ...
                sb.AppendLine("EXECUTE GenerarReportePorCliente @clie_Id");
                SqlCommand cmd = new SqlCommand(sb.ToString(), cn);
                cmd.Parameters.AddWithValue("@clie_Id", int.Parse(clie_Id.ToString()));
